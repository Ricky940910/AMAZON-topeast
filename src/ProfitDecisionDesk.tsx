      ...
      <div className="sim-page">
        ...
        {activeTab === "timeline" && <div className="sim-timeline-layout">
          <aside className="sim-card sim-month-editor">
            ...
            <div className="sim-month-form">{monthField("售价", "listPrice", { prefix: symbol })}{monthField("销量", "orders", { suffix: "单" })}{monthField("Coupon", "couponRate", { percent: true, suffix: "%" })}{monthField("广告订单占比", "adOrderShare", { percent: true, suffix: "%" })}{input.sales.advertisingMode === "acos" ? monthField("ACOS", "acos", { percent: true, suffix: "%" }) : monthField("广告花费", "adSpend", { prefix: symbol })}{monthField("退货率", "returnRate", { percent: true, suffix: "%" })}{monthField("FBA Fee", "fbaFee", { prefix: symbol })}{monthField("产品成本 / 件", "productCostPerUnit", { prefix: symbol })}{monthField("物流成本 / 件", "logisticsCostPerUnit", { prefix: symbol })}{monthField("其他推广成本", "otherPromotionCost", { prefix: symbol })}<button className="sim-clear-month" type="button" onClick={clearMonthlyOverride}><RefreshCcw size={14} /> 恢复基础值</button></div>
